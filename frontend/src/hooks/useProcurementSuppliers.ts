...
import {
  ...
  type SupplierSearchRunResult,
} from "@/services/api";
import { getProcurementErrorMessage } from "@/lib/errors";
import {
  filterSuppliersByThreshold,
  getCoverageInfo,
  sortSuppliersByCoverage,
  type SearchSupplier,
} from "@/lib/suppliers";
...
const parseSupplierSearchPayload = (
  value: unknown,
  depth = 0,
): SupplierSearchResult["parsed"] | null => {
  ...
  if (Array.isArray(record.suppliers)) {
    return record as SupplierSearchResult["parsed"];
  }
...
export function useProcurementSuppliers({
  ...
  suppliers,
  t,
}: UseProcurementSuppliersParams) {
  const searchPollingTimeoutRef = useRef<number | null>(null);
  const activeSearchRunIdRef = useRef<string | null>(null);
  const searchSessionRef = useRef(0);
  const [searchResult, setSearchResult] = useState<SupplierSearchResult | null>(
    null,
  );
  const [isSearchingSuppliers, setIsSearchingSuppliers] = useState(false);
  const [isTransferringSuppliers, setIsTransferringSuppliers] = useState(false);
  const [supplierSearchError, setSupplierSearchError] = useState<string | null>(
    null,
  );
  const [selectedSearchSuppliers, setSelectedSearchSuppliers] = useState<
    Set<number>
  >(new Set());
  ...
  const applyCompletedSearchResult = useCallback(
    (result: SupplierSearchRunResult | null | undefined) => {
      if (!result) {
        return;
      }

      const normalizedParsed = parseSupplierSearchPayload(
        result.parsed ?? result.raw ?? null,
      );
  ...
  const cancelSearchRun = useCallback(
    async (searchRunId: string) => {
      try {
        await supplierSearchCancel(
          buildPayload("korus_ai_procurement__supplier_search_cancel", [
            searchRunId,
          ]),
        );
      } catch (error) {
  ...
  const pollSearchStatus = useCallback(
    async (searchRunId: string, sessionId: number) => {
      try {
        const result = await supplierSearchStatus(
          buildPayload("korus_ai_procurement__supplier_search_status", [
            searchRunId,
          ]),
        );
        ...
        setSupplierSearchError(
          result.error || t("errors.searchFailedDescription"),
        );
        ...
        setSearchResult(null);
        setSupplierSearchError(
          getProcurementErrorMessage(error, t("errors.searchFailed")),
        );
        toast.error(t("errors.searchFailedTitle"), {
          description: getProcurementErrorMessage(
            error,
            t("errors.searchFailedDescription"),
          ),
        });
      }
    },
  ...
  const handleSearchSuppliers = useCallback(async () => {
    ...
    try {
      const result = await supplierSearchStart(
        buildPayload("korus_ai_procurement__supplier_search_start", [
          packageId,
        ]),
      );
  ...
  const sortedSearchSuppliers = useMemo(
    () =>
      sortSuppliersByCoverage(
        searchResult?.parsed?.suppliers ?? [],
        itemsCount,
      ),
  ...
  const filteredSearchSuppliers = useMemo(
    () =>
      filterSuppliersByThreshold(
        sortedSearchSuppliers,
        itemsCount,
        savedCoverageThreshold,
      ),
    [sortedSearchSuppliers, itemsCount, savedCoverageThreshold],
  );

  const handleAddSupplierFromSearch = useCallback(
    async (supplier: SearchSupplier) => {
      ...
      const matchedItemsJson =
        supplier.matchedItems && supplier.matchedItems.length > 0
          ? JSON.stringify(supplier.matchedItems)
          : null;

      const created = await addManualSupplier(
        buildPayload("korus_ai_procurement__supplier_add_manual", [
          packageId,
          supplier.name,
          supplier.url || null,
          supplier.email || null,
  ...
  const handleToggleSupplier = useCallback(
    async (supplier: ProcurementSupplier) => {
      try {
        const updated = await selectSupplier(
          buildPayload("korus_ai_procurement__supplier_select", [
            supplier.id,
            !supplier.selected,
          ]),
        );
        ...
        setSuppliers((previousSuppliers) =>
          previousSuppliers.map((currentSupplier) =>
            currentSupplier.id === updated.id ? updated : currentSupplier,
          ),
        );
      } catch (error) {
        toast.error(t("errors.toggleSupplierFailedTitle"), {
          description: getProcurementErrorMessage(
            error,
            t("errors.toggleSupplierFailedDescription"),
          ),
        });
      }
    },
    ...
    async (supplierId: string) => {
      try {
        await deleteSupplier(
          buildPayload("korus_ai_procurement__supplier_delete", [supplierId]),
        );
        ...
        toast.error(t("errors.deleteSupplierFailedTitle"), {
          description: getProcurementErrorMessage(
            error,
            t("errors.deleteSupplierFailedDescription"),
          ),
        });
      }
    },
  ...
  const handleToggleSearchSupplier = useCallback((index: number) => {
  ...
  const handleTransferSelectedSuppliers = useCallback(async () => {
    ...
    try {
      ...
      const selectedSuppliers = filteredSearchSuppliers.filter((_, index) =>
      ...
      toast.error(t("errors.transferSuppliersFailedTitle"), {
        description: getProcurementErrorMessage(
          error,
          t("errors.transferSuppliersFailedDescription"),
        ),
      });
      return false;
    } finally {
