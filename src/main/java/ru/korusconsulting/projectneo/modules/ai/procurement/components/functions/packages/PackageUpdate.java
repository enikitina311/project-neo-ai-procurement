...
public class PackageUpdate extends DataFunctionBase implements DataFunction {
    ...
    public String description() {
        return "Update procurement package: id, project_id, name, criteria_text?, coverage_threshold?, suppliers_limit?";
    }

    @Override
    public Object execute(ComponentArgs args) {
        ProcurementFunctionArgs.requireMinArgs(args, 3, "Expected at least 3 arguments: id, project_id, name");

        ProcurementPackageDtoRequest request = new ProcurementPackageDtoRequest();
        request.setId(ProcurementFunctionArgs.uuid(args, 0));
        request.setProjectId(ProcurementFunctionArgs.uuid(args, 1));
